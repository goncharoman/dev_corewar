...
/*   Updated: 2020/06/17 05:26:18 by ujyzene          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			uint8_t index)
{
	...
	value = 0;
	if (cursor->oper_args_types[index] == T_REG)
		value = cursor->reg[get_byte(mem, cursor->pc + *offset) - 1];
	else if (cursor->oper_args_types[index] == T_DIR)
		value = get_value(mem, cursor->pc + *offset,
			cursor->oper->tdir_size);
	else if (cursor->oper_args_types[index] == T_IND)
		value = get_value(mem,
