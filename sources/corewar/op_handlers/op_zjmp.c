...
/*   Updated: 2020/05/18 03:29:33 by ujyzene          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <corewar_op.h>

inline static void	log_oper(int32_t id, int32_t addr, t_bool jump)
{
	ft_printf("P %4d | zjmp %d %s\n", id + 1, addr, jump ? "OK" : "FAILED");
}
...
uint32_t	handler_zjmp(t_vm *vm, t_cursor *cursor)
{
	...
	if (vm->log_lavel & LOG_OPER)
		log_oper(cursor->id, addr, cursor->jump);
	return (offset);
}
